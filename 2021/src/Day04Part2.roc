...
# Str.toNum should be in standard library right now,
# don't copy this function the next time!
...
partition : List elem, (elem -> Bool) -> { yes: List elem, no: List elem }
partition = \list, pred ->
    acc, elem <- List.walk list { yes: [], no: [] }

    if pred elem then
        { acc & yes: List.append acc.yes elem }
    else
        { acc & no: List.append acc.no elem }


WinnerBoard : { board: Board, lastDrawnNum: U64 }

findWinners : List Board, List U64 -> { boards: List Board, winners: List WinnerBoard }
findWinners = \startBoards, drawnNums ->
    acc, num <- List.walk drawnNums { boards: startBoards, winners: [] }

    { yes, no: others } = markBoards acc.boards num |> partition winningBoard
    newWinners = List.map yes \board -> { board, lastDrawnNum: num }

    { boards: others, winners: List.concat acc.winners newWinners }
...
runGame = \handle ->
    ...
    { winners } = findWinners boards drawnNums

    { board, lastDrawnNum } <- List.last winners
        |> Result.mapErr (\ListWasEmpty -> NoWinnerFound)
        |> Task.fromResult
        |> Task.await
    ...
    {} <- Task.await (Stdout.line "Last winner")
