...
updateCoords = \coords, cmd ->
    { position, depth } = coords

    value : I64
    value = Num.intCast cmd.value

    when cmd.direction is
        Forward -> { coords & position: position + value }
        Down -> { coords & depth: depth + value }
        Up -> { coords & depth: depth - value }
...
main =
    ...
    result <- Task.attempt (runCommands handle { position: 0, depth: 0 })
    ...
    message =
        when result is
            Ok coords ->
                ...
                "Result: position = \(position), depth = \(depth)\nposition * depth = \(mult)"
